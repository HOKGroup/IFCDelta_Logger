...
namespace Revit.IFC.Export.Exporter
{
    ...
    public class IFCHeaderData
    {

        public string ModelID { get; set; }
        public string ModelVersionId { get; set; }
        public string PreviousModelVersionID { get; set; }
    ...
    {
        public static List<Element> allExportedEle = new List<Element>();
        private List<ElementId> allScopedElementsIds { get => allExportedEle.Select(x => x.Id).ToList(); }


        public static bool ExportIFCperElement { get; set; }

        List<Element> preExportElements = new List<Element>();
        public static string logPath = "";
        public static IFCHeaderData IfcHeaderData;
        ...
        public void ExportIFC(Autodesk.Revit.DB.Document document, ExporterIFC exporterIFC, Autodesk.Revit.DB.View filterView)
        {
            ...
            try
            {
                ...
                if (ExportIFCperElement)
                {
                    ...
                    InitializeElementExporters();
                    if (m_ElementExporter != null)
                        m_ElementExporter(exporterIFC, document, true);
                    ...
                    foreach (var element in preExportElements)
                    {
                        if (!allScopedElementsIds.Contains(element.Id))
                        ...
                        EndExport(exporterIFC, document);
                        if (exported)
                            WriteIFCFile(exporterIFC, document, ExportIFCperElement, element);
                        statusBar.Set(string.Format("Writring IFC File of element {0} of {1}..", counter, allScopedElementsIds.Count));
                        ExporterCacheManager.Clear();
                        ExporterStateManager.Clear();
                        counter++;
                    }
                ...
                else
                {
                    ...
                    InitializeElementExporters();
                    if (m_ElementExporter != null)
                        m_ElementExporter(exporterIFC, document, ExportIFCperElement);
        ...
        /// <param name="exporterIFC">The IFC exporter object.</param>
        /// <param name="document">The document to export.</param>
        /// <param name="filePerIFCelement"></param>
        private void WriteIFCFile(ExporterIFC exporterIFC, Document document, bool filePerIFCelement, Element element = null)
        {
            ...
            using (IFCTransaction transaction = new IFCTransaction(file))
            {
                ...
                if (element?.Id == new ElementId(153616))
                {

                }

                string fileNameWithoutExtension = Path.GetFileNameWithoutExtension(exportOptionsCache.FileName);
                string dir = logPath + $@"\IFC\{document.Title}\{date.ToString("yyyyMMddHHmmss")}\";
                ...
                if (filePerIFCelement)
                {
                    ...
                    if (string.IsNullOrEmpty(ifcclassName) || ifcclassName.ToLower().Contains("curtain"))
                    {
                        return;
                    }
                ...
                {
                    writeOptions.FileName = Path.Combine(dir, document.Title+ $"_{date.ToString("yyyyMMddHHmmss")}_delta.ifc");
                ...
                if (!ExportIFCperElement)
                {
                    var allLines = File.ReadAllLines(writeOptions.FileName).ToList();
                    var index = allLines.FindIndex(x => x.Contains("ENDSEC;"));
                    List<string> injectedLines = new List<string>();
                    if (index != -1)
                    {
                        injectedLines.Add("[");
                        if (!string.IsNullOrEmpty(IfcHeaderData.ModelID))
                        {
                            injectedLines.Add("ModelID:" + IfcHeaderData.ModelID);

                        }
                        if (!string.IsNullOrEmpty(IfcHeaderData.ModelVersionId))
                        {
                            injectedLines.Add("ModelVersionId:" + IfcHeaderData.ModelVersionId);

                        }
                        if (!string.IsNullOrEmpty(IfcHeaderData.PreviousModelVersionID))
                        {
                            injectedLines.Add("PreviousModelVersionID:" + IfcHeaderData.PreviousModelVersionID);

                        }

                        if (IfcHeaderData.DeletedItems.Count > 0)
                        {
                            var str = "";
                            foreach (var deletedItem in IfcHeaderData.DeletedItems)
                            {
                                str += deletedItem + ", ";
                            }
                            ...
                            injectedLines.Add("DeletedItems[" + str + "]");
                        }

                        injectedLines.Add("]");

                        var header = allLines.Take(index + 1).ToList();
                        var afterHeader = allLines.Skip(index + 1);
                        var newFileStrLines = header;
                        newFileStrLines.AddRange(injectedLines);
                        newFileStrLines.AddRange(afterHeader);


                        File.WriteAllLines(writeOptions.FileName, newFileStrLines);
