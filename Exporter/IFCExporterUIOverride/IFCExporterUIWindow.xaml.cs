...
namespace BIM.IFC.Export.UI
{
   ...
   public partial class IFCExporterUIWindow : ChildWindow
   {
      ...
      private void buttonLoadSetup_Click(object sender, RoutedEventArgs e)
      {
       ...
       var allFiles =   Directory.GetFiles(logpath + @"\IFC\Settings").ToList();
         Regex rx = new Regex(@"(IFC Configuration)[\w\s\b\t\r\W]+(\.json)");

         var IFCConfiguration = allFiles.Where(x => rx.IsMatch(x)).FirstOrDefault();

         // Get the selected file name and display in a TextBox 
         //if (result.HasValue && result.Value)
         if (!string.IsNullOrEmpty(IFCConfiguration))
         {
            try
            {
              // using (StreamReader sr = new StreamReader(openFileDialog.FileName))
               using (StreamReader sr = new StreamReader(IFCConfiguration))
               {
            ...
            {
               TaskDialog.Show("Error", "An Error happened during reading IFC Configuration File.\nPlease make sure it is a valid file.");
      ...
      public void LoadConfiguaration(string LogPath)
      {
         this.logpath = LogPath;
         buttonLoadSetup_Click(null, null);
